...
class Tiger extends Animal{
    // 필드
    public name : string; // public이 디폴트
...
const tiger = new Tiger('crk',36,'충주');
tiger.bark();
...
/*
    3. Generics
     ...
     - Type에 유연성을 제공하면서 확장성과 타입안전성을 추가할 수 있다.
...
const result = returnGeneric<string>("hello");
...
/*
    3) 제네릭의 기본타입
     - 제네릭이 사용되지 않는 경우 ,타입변수에 대입될 기본값 설정
*/
...
const arr3 = defValue(); // 기본값 string[], 기본값사용
...
function Config<T extends Options = {theme:'dark'}>(options?:T):T{
    ...
    // return options || {theme:"dark"} as T;  // options true하다면 options false하다면 theme:dark
...
/*
    3) Omit<T,K>
     - 객체타입에서 특정 속성을 제외한 나머지만 남긴 새 타입을 만들때 사용
*/
type NewUser = Omit<User,"id">; 
const userData:NewUser = {name:"crk",email:"zz"};

/*
    Utility Type 조합
    1. User에서 id를 제외한 나머지 속성을 선택한 후 전체를 선택속성으로 만들기
    ...
    {name:string} &
    {name>:string , email?:string} 
    - 같은 속성이 병합되는 경우 ,좀더 구체적인 속성으로 병합
*/
...
*/
function getUser(){
    return {id:1,name:'crk',email:"aaa"};
}
