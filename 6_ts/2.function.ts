/*
    1. 함수타입
     ...
     - 저장된 타입과 다른 값이 전달되거나, 반환되면 컴파일 에러발생
*/
...
/*
    3. Optional Parameter
     ...
     - 변수명?:type
     - Optional Parameter는 type | undefined 유니언 타입으로 설정
*/
...
/*
    ...
    갯수가 고정된 일반함수에서 전개연산자를 통해 인자를 전달할 수 없다.
    -> 자릿수가 정해진 튜플로 해결 가능
*/
//normalFn(...spreadArr); X
const arr2 = [1,2] as const;
normalFn(...arr2);

/*
    6. Destructuring (비구조화 문법)
...
objectDesFn2({a:111,b:'crk',c:false});

/*
    7. 타입 좁히기(Type narrowing)
     - unionType이나 unknown처럼 변수가 여러 타입을 가지고 
     있을 때, 이를 사용하는 시점에서 특정 한 가지로 좁히는 문법
...
/*
    8. Type Assertion(타입단언)
     - 여러 타입을 가질 수 있는 변수에 대하여 "개발자가 직접"
     해당 값의 타입을 명확히 지정하는 문법
     - 타입단언시 컴파일러가 이를 믿고 타입 체크를 생략
     - 타입단언은 개발자가 실제 타입을 완벽하게 알고 있을때만 사용한다
*/
...
/*
    9. never type
     - 어떤 값도 가질 수 없는 타입을 의미하며, 절대 값을 
     반환할 수 없는 함수나, 도달할 수 없는 코드블럭을 표현할때 사용
