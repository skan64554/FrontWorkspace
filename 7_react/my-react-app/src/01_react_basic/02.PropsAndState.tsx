   ...
   let [message, setMessage] = useState('Hello From Parent');
    ...
    /*
        #2. State
         ...
         - 좋아요 상태, 팔로우 숫자, 입력하는 값 등 컴포넌트내부에서 이 상태값을 감지하기 위한 용도로 선언
         - 부모컴포넌트에서 여러 자식 컴포넌트에 공유해야할 데이터가 있는 경우
         부모컴포넌트에서 state를 선언후 자식에게 props로 전달해야한다
    */
...
function ChildComponent({style, message, setMessage}:ChildProps){
    //  console.log(props);
    // props.message = "마음에 안드네..." // props는 읽기전용으로 사용할 것
    ...
    return (<div>
        ...
        <button onClick={messageHandler}>messageHandler</button>
