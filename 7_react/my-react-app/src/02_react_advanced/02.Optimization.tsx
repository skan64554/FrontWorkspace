...
export default function OptimizationHook(){
    /*
        #1. userMemo
         - 계산결과를 메모해두는 훅 함수
         - 리렌더링시 계산비용이 큰 작업의 결과를 캐싱하여 렌더링을 최적화한다.
   ...
   const heavyComputation = (list: number[]) => {
    ...
    // useMemo를 활용한 배열 캐싱
    // 현재 컴포넌트가 렌더링 될 때마다 배열을 정렬시키고 있는데,
    ...
    /*
        #2. useCallback
         ...
         함수가 새롭게 생성되면서 자식 컴포넌트 또한 리렌더링 되는데 이를 방지하기 위해 사용한다
         - React.memo와 함께 사용
...
// React.memo -> props가 바뀌지 않으면 리렌더링 하지 않는 컴포넌트로 만드는 함수
