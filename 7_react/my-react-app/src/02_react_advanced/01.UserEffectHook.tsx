...
export default function UseEffectHook(){
    /*
        #1. Hook
         ...
         - 원래 React에서는 컴포넌트가 복잡한 동작을 수행하려면 클래스 컴포넌트를 써야했지만
         그럴 경우 코드가 복잡하고 가독성이 떨어졌음
   ...
   /*
        #2. useEffect
         ...
         업데이트(update)될 때를 감지하여 특정 작업을 수행할 수 있게
         도와주는 hook 함수

        #3_1) 생명주기 함수
         - 컴포넌트는 생성부터 소멸까지 특정 순서에 맞춰서 동작하며, 
         이 과정을 생명주기(lifecycle)이라고 부른다
         - mount : 컴포넌트 첫 렌더링 시기
         - update : props나 state가 변경되어 컴포넌트가 리렌더링되는 시기
        ...
        },[의존성,의존성...]) // 의존성(state)이 변경될 때마다 useEffect실행
   */
