...
document.querySelector("#id").onclick = function(){
    ...
    // 2. querySelector('css선택자') css 선택자와 일치하는 요소 1개 선택
    ...
    // 3. id속성값으로 접근하기  (권장 X)
    // - 브라우저는(몇몇 브라우저 제외) id값의 요소를 전역변수 형태로 등록
    // 하기 때문에 id 속성값을 통해 요소를 바로 획득할 수 있다.
...
/*
    #userEmail을 입력하고, 확인을 눌렀을때 사용자 입력값 출력
    - 아이디로 접근
    - 태그명으로 접근
    - name값으로 접근
*/
...
document.querySelector("#email-confirm").onclick = function(){
    ...
    // name값으로 접근
    var userEmail2 = document.querySelector('[name=userEmail]');
