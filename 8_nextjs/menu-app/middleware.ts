...
/*
    #1. Middleware
     - 서버컴포넌트가 렌더링 되거나, API가 실행되기 전에 요청을 가로채서
     특정 로직을 수행할 수 있는 서버측 도구 (필터역할수행)
...
export async function middleware(request:NextRequest){
    ...
    /*
        1. 액세스 토큰은 만료되어 없으나, 리프레쉬 토큰만 남아 있는 경우
         - 그대로 api요청을 하면 401에러가 발생하므로, middleware에서 쿠키 재발급요청을
         보낼 예정이다.
    */
   if(!accessToken && refreshToken){
        const refreshRes = await fetch(`http://locahost:8081/api/auth/refresh`,
            {
                method:'POST',
        ...
        if(refreshRes.ok){
            ...
            if(setCookie){
                ...
                // refresh 실패시 쿠키 삭제 후 로그인 페이지 리디렉트
    ...
    // 1) 비로그인 사용자(/menus/new, /menus/:id ... 이용 불가)
        ...
        const isProtectedPage = protectedPathPrefixes.some
...
/*
    middleware 실행 범위 설정
     - 모든 요청에 몸들웨어가 동작하면 성능이 저하된다.
     - 특정 경로에서만 미들웨어가 작동하도록 범위제한이 필수
*/
export const config = {
    matcher : [
