...
import { useUserInfoActions } from '../hooks/useUserInfoAction';
    ...
    const handleLogin = async (e:React.SubmitEvent) => {
        ...
        .then( response => {
            ...
            /*
                #1. JWT
                 ...
                 - JWT를 활용한 인증/인가 매커니즘은 세션기반 로그인방식과 달리 서버거 인증상태를 따로 
                ...
                #2. JWT저장 위치
                 ...
                 1) localStorage
                  ...
                  - XSS공격에 취약하다. 따라서 JWT와 같은 중요한 토큰은 보관하지 않는다.

                 2) sessionStorage
                  - 탭 단위 세션 저장소로 탭을 닫으면 데이터도 삭제된다. (데이터유지시간이 짧다)
                  - XSS공격에 취약하다. 

                 3) 쿠키
                  - 쿠키는 특정 도메인의 경로에 지정된 시간동안만 저장되는 데이터
                  - 쿠키는 HTTP통신시 항상 자동으로 전달된다.
                  - 기본 쿠키는 JS로 접근하여 탈취될 수 있느나 http-only 설정시 js로 쿠기 제어 불가능
                  (XSS에서 안전)

                 4) 리액트메모리
                  - REDUX에 보관시 메모리에만 저장되며, 새로고침시 데이터가 소멸한다.
                  - XSS공격에 위험이 존재하긴 하나 유지시간이 굉장히 짧기 때문에 공격을 최소화한다.

                #3. AccessToken과 RefreshToken
                 - JWT기반 인증에서 사용하는 토큰들.
                 - AccessToken실제 인증에 사용하는 토큰으로, 유지시간을 짧게 설정하여 
                 탈취시 부담을 줄여야한다.
        ...
        .catch((err:AxiosError) => {
            ...
            if(err.response?.status === 404){
                // 등록된 계정이 없는 경우
                const doSignUp = confirm("등록된 계정이 없습니다. 회원가입 하시겠습니까?");
                if(!doSignUp){
                    setError("계정을 다시 한 번 확인해주세요");
                    return;
                }
