...
export default async function MenuListPage(){
    /*
        #1. 메뉴리스트
         - Next.js에서 화면작업을 할때는, 데이터베이스에서 조회가 필요한 부분과,
         필요없는 부분을 나눠서 각각 서버 컴포넌트, 클라이어느 컴포넌트로 구분해줘야 한다
         - page.tsx는 서버컴포넌트로 관례상 항상 추가해야하며, 데이터베이스 조회작업 및
         공용 레이아웃(layout.tsx)을 작성한다
    */

    /*
        #2. fetch
         - Next.js의 서버컴포넌트에서 사용하는 fetch는 브라우저의 기본 fetch를
         확장한 함수로, 단순히 데이터를 가져오는 것을 넘어 데이터를 캐싱하고 재사용하는
         기능을 추가했다
         - 따라서 같은 api를 여러 컴포넌트에서 호출해도 실제 네트워크 요청은 한번만
         발생하여 기능을 최적화 할 수 있다.
         - fetch함수에 cache속성을 추가하지 않는 경우 cache:auto속성이 적용된다
            cache:auto는 기본 static render방식으로 페이지를 서버스한다
        ...
        #3. fetch의 옵션별 렌더링 방식
         ...
         2) cache : no-store
          - 실시간 데이터가 중요한 환경에서 사용하는 속성으로 매 요청시마다
          데이터를 새로 가져오는 방식
